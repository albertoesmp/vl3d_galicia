...
def export_sql_class_distributions(scenes, outf):
    ...
    export_funs = [
        ...
        _export_class_distribution_fullraw
    ]
    for i, scene in enumerate(scenes):
...
def _export_class_distribution_fullraw(scene, outf, write_comma):
    dataset_name = f"{scene['name'][:-4]}_FULLRAW"
    class_keys = ['ground', 'water', 'lowveg', 'midveg', 'highveg', 'building', 'ignore']
    assoc_class_keys = [
        ['ground'],
        ['water'],
        ['lowveg'],
