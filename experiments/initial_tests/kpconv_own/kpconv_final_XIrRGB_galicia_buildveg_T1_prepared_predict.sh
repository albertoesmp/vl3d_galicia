#!/bin/bash
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -c 32
#DISABLED: SBATCH --gres=gpu:a100
#SBATCH -t 24:00:00
#SBATCH --mem 123G
#SBATCH --mail-type=begin
#SBATCH --mail-type=end
#SBATCH --mail-user=albertoesmp@gmail.com

# Author: Alberto M. Esmoris Pena
# Brief: Script to train models with VL3D framework


# ---  PREPARE ENVIRONMENT  --- #
# ----------------------------- #
source /mnt/netapp1/Store_usccimyg/Codigos/vl3d_galicia_dev/cesga/vl3d_cesga_env.sh


# ---  VARIABLES  --- #
# ------------------- #
PREDICTIVE_SPEC='/home/usc/ci/aep/git/vl3dpp/cesga/galicia/buildveg/kpconv/kpconv_final_XIrRGB_buildveg_prepared_predict_T1.json'

# ---  EXECUTION  --- #
# ------------------- #
# RUN SCRIPTS
cd_vl3d
#srun python ${VL3D_SCRIPT} --test
srun python ${VL3D_SCRIPT} --pipeline ${PREDICTIVE_SPEC}
