...
from src.model.deeplearn.arch.spconv3d_pwise_classif import \
...
# ---   CLASS   --- #
# ----------------- #
class SpConv3DPwiseClassifModel(ClassificationModel):
    """
    :author: Alberto M. Esmoris Pena

    Sparse 3D convolutional model for classification tasks.
    See :class:`.ClassificationModel`.

    :ivar model: The deep learning model wrapped by the corresponding handler,
        i.e., the :class:`.SpConv3DPwiseClassif` model wrapped by a
    ...
    def extract_model_args(spec):
        ...
        SpConv3DPwiseClassifModel from a key-word specification.
        ...
        # Extract particular arguments for SpConv3DPwiseClassif models
    ...
    def __init__(self, **kwargs):
        ...
        :param kwargs: The attributes for the SpConv3DPwiseClassifModel
        ...
        if not 'fnames' in kwargs:
            if 'model_args' in kwargs and 'fnames' in kwargs['model_args']:
                ...
                kwargs['fnames'] = ['ones']  # Avoid None fnames exception
        super().__init__(**kwargs)
        # Basic attributes of the SpConv3DPwiseClassifModel
    ...
    def prepare_model(self):
        """
        Prepare a sparse 3D convolutional point-wise classifier with current
        model arguments.

        :return: The prepared model itself. Note it is also assigned as the
            model attribute of the object/instance.
        :rtype: :class:`.SpConv3DPwiseClassifModel`
        """
        ...
        if self.model is None:
            if self.model_args is not None:
                ...
                self.model = SpConv3DPwiseClassif(**self.model_args)
            else:
                LOGGING.LOGGER.info(
                    "Preparing a SpConv3DPwiseClassifModel with no "
                    "`model_args`"
                )
                self.model = SpConv3DPwiseClassif()
        else:
            LOGGING.LOGGER.debug(
                'Preparing a pretrained SpConv3DPwiseClassifModel.'
            )
            return self.model
    ...
    def get_input_from_pcloud(self, pcloud):
        ...
        if self.fnames is None:
            return [
                X,
                np.ones((X.shape[0], 1))  # SpConv3D always needs features
            ]
    ...
    def training(self, X, y, F=None, info=True):
        """
        The fundamental training logic to train a sparse 3D convolutional
        ...
        if info:
            LOGGING.LOGGER.info(
                f'SpConv3DPwiseClassifModel trained in {end-start:.3f} seconds.'
            )
        ...
        if F is None:
            if len(X) < 2:
                raise DeepLearningException(
                    'SpConv3DPwiseClassifModel cannot work without features. '
    ...
    # ---  SPCONV3D PWISE CLASSIF METHODS  --- #
    # ---------------------------------------- #
    def compute_pwise_activations(self, X, reducer=None):
        ...
        3D sparse convolutional point-wise classification model.
