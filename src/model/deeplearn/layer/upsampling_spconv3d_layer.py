...
class UpsamplingSpConv3DLayer(Layer):
    ...
    def build(self, dim_in):
        ...
        if self.h is None:
            raise DeepLearningException(
                'UpsamplingSpConv3DLayer failed to build mutable hash table.'
            )
    ...
    def call(self, inputs, training=False, mask=False):
        ...
        :param inputs: The feature space matrices as a 3D tensor with padding,
            the submanifold map :math:`h` for the non-upsampled sparse grid
            ...
            values, the :math:`h^U` vector of indices that gives the indices
            of the active cells in the non-upsampled sparse grid that are
            the centers for the upsampling convolutional windows, and the
            ...
            of the map h for the sparse grid before the upsampling,
            ...
            grid, and inputs[4] the axis-wise partitions.
        :return: The output feature space matrices as a tensor with padding.
        :rtype: :class:`tf.Tensor`
        """
        def convolve(input):
            # Extract inputs
            F, hk, hv, hU, n, start, src_start = input
