...
class FeaturesDownsamplingLayerTest(VL3DTest):
    ...
    def run(self):
        ...
        fdl_exp = FeaturesDownsamplingLayer(filter='exponential')
        fdl_mean = FeaturesDownsamplingLayer(filter='mean')
        fdl_nearest = FeaturesDownsamplingLayer(filter='nearest')
        # Compute gaussian and mean downsampling
        with tf.device("cpu:0"):
            fdl_gauss_out = fdl_gauss.call(inputs)
            fdl_exp_out = fdl_exp.call(inputs)
        ...
        valid = valid and self.validate_exponential_downsampling(
            inputs, num_encoding_neighbors, fdl_exp_out
        )
        valid = valid and self.validate_nearest_downsampling(
    ...
    def validate_exponential_downsampling(
        self, inputs, nneighs, fdl_exp_out
    ):
        """
        Check whether the exponential-based downsampling layer yielded the
        expected output (True) or not (False).

        :param inputs: The inputs to the layer.
        :param nneighs: The number of encoding neighbors.
        :param fdl_exp_out: The output generated by the layer.
        :return: True if the output is okay, False otherwise.
        """
        X_batch, Xb_batch, F_batch, ND_batch = inputs
        num_elems_in_batch = X_batch.shape[0]
        fdl_exp_out = np.array(fdl_exp_out)
        for k in range(num_elems_in_batch):
            X, Xb, F, ND = X_batch[k], Xb_batch[k], F_batch[k], ND_batch[k]
            kdt = KDT(X)
            I = kdt.query(Xb, k=nneighs)[1]
            ...
            omega_sq = np.max(D_sq, axis=1)
            exps = np.exp(D_sq/np.expand_dims(omega_sq, 1))
            exp_norms = np.sum(exps, axis=1)
            ref_out = np.sum(np.expand_dims(exps, axis=2)*F[I], axis=1)
            ref_out = ref_out / np.expand_dims(exp_norms, axis=1)
            if not np.allclose(fdl_exp_out[k], ref_out, atol=self.eps, rtol=0):
                return False
        return True

    def validate_nearest_downsampling(
        self, inputs, nneighs, fdl_nearest_out
    ):
        """
        Check whether the nearest neighbor downsampling layer yielded the
        expected output (True) or not (False).
