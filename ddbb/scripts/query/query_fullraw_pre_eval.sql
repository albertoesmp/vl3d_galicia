...
-- Brief: SQL query to compute the pre-evaluation of ground, water, building,
--          and vegetation by height experiments. The pre-evaluation considers
--          only a few point clouds instead of the full dataset.
...
WHERE datasets.name like any(array['%MERGE\_195\_%', '%MERGE\_212\_%', '%MERGE\_108\_%', '%MERGE\_13\_%'])
        and datasets.name like '%norm\_FULLRAW'
GROUP BY models.id
ORDER BY "F1" desc
